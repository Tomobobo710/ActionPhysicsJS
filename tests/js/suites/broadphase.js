...
(function (Runner) {
	...
	function test(group, name, fn, opts) {
		opts = opts || {};
		Runner.test(group, name, fn, { page: 'broadphase', description: DESC, visual: !!opts.visual, steps: 0 });
	}

	function mkBody(shape, mass, pos, color) {
		var b = new AP.RigidBody(shape, mass);
		b.position.set(pos[0], pos[1], pos[2]);
		b.updateDerived();
		if (color) b._color = color;
		return b;
	}
	...
	test('collision/broadphase', 'two overlapping spheres produce one candidate pair', function (t) {
		...
		var b = mkBody(new AP.SphereShape(1), 1, [1.5, 0, 0]); // distance 1.5 < radius sum 2
		bp.add(a); bp.add(b);
	...
	test('collision/broadphase', 'two far-apart spheres produce no pair', function (t) {
		var bp = new AP.SAPBroadphase();
		var a = mkBody(new AP.SphereShape(1), 1, [0, 0, 0]);
		var b = mkBody(new AP.SphereShape(1), 1, [10, 0, 0]);
		bp.add(a); bp.add(b);
	...
	test('collision/broadphase', 'AABBs overlapping only off the sweep axis are still caught', function (t) {
		...
		var bp = new AP.SAPBroadphase();
		var wide1 = mkBody(new AP.SphereShape(0.1), 1, [-50, 0, 0]);
		var wide2 = mkBody(new AP.SphereShape(0.1), 1, [50, 0, 0]);
		var a = mkBody(new AP.SphereShape(1), 1, [0, 0, 0]);
		var b = mkBody(new AP.SphereShape(1), 1, [0, 1.5, 0]);
	...
	test('collision/broadphase', 'two static bodies never pair with each other', function (t) {
		var bp = new AP.SAPBroadphase();
		var a = mkBody(new AP.SphereShape(1), 0, [0, 0, 0]);   // static
		var b = mkBody(new AP.SphereShape(1), 0, [0.5, 0, 0]); // static, overlapping AABB
		bp.add(a); bp.add(b);
	...
	test('collision/broadphase', 'a dynamic body still pairs against an overlapping static one', function (t) {
		var bp = new AP.SAPBroadphase();
		var ground = mkBody(new AP.BoxShape(10, 0.5, 10), 0, [0, -0.5, 0]);
		var falling = mkBody(new AP.SphereShape(1), 1, [0, 0, 0]);
	...
	test('collision/broadphase', 'collision mask/group filters a pair before it reaches midphase', function (t) {
		var bp = new AP.SAPBroadphase();
		var a = mkBody(new AP.SphereShape(1), 1, [0, 0, 0]);
		var b = mkBody(new AP.SphereShape(1), 1, [0.5, 0, 0]);
		...
		var c = mkBody(new AP.SphereShape(1), 1, [0, 0, 0]);
		var d = mkBody(new AP.SphereShape(1), 1, [0.5, 0, 0]);
	...
	test('collision/broadphase', 'candidate pairs among a small cluster of bodies', function (t) {
		var a = mkBody(new AP.SphereShape(1), 1, [0, 0, 0], '#4af');
		var b = mkBody(new AP.SphereShape(1), 1, [1.5, 0, 0], '#4af');
		var c = mkBody(new AP.SphereShape(1), 1, [0, 3, 0], '#4af');
		var d = mkBody(new AP.SphereShape(1), 1, [6, 0, 0], '#666');
		t.bodies.push(a, b, c, d);
		var bp = new AP.SAPBroadphase();
